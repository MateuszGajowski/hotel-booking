...
public interface HotelRepository extends JpaRepository<HotelEntity, Long> {
    @Query("SELECT distinct h FROM hotel h JOIN FETCH h.rooms r LEFT JOIN FETCH r.bookings b WHERE h.city = :city AND " +
            "r.type.price >= :startPrice AND r.type.price <= :endPrice " +
            "AND (NOT EXISTS (SELECT b from b WHERE b.dateIn >= :dateIn AND b.dateOut <= :dateOut AND b.status != 2))")
